...
import { Extent, GlobeLayer, TiledGeometryLayer } from 'Main';

const dimensions = new THREE.Vector2();

function setTileFromTiledLayer(tile: TileMesh, tileLayer: TiledGeometryLayer) {
    if (tileLayer.diffuse) {
        tile.material.setUniform('diffuse',
            new THREE.Color(...tileLayer.diffuse));
    }

    // @ts-expect-error: global debug mode constexpr
    if (__DEBUG__) {
        tile.material.setUniform('showOutline', tileLayer.showOutline || false);
    }

    if (tileLayer instanceof GlobeLayer) {
        ...
        tile.horizonCullingPoint =
            tile.extent.center().as('EPSG:4978').toVector3();
        tile.extent
        ...
        tile.horizonCullingPoint
            .setLength(h * tile.horizonCullingPoint.length());
        tile.horizonCullingPointElevationScaled =
            tile.horizonCullingPoint.clone();
    }
}

export default {
    convert(
        requester: TileMesh | undefined,
        extent: Extent,
        layer: TiledGeometryLayer,
    ) {
        const builder = layer.builder as TileBuilder<TileBuilderParams>;
        const parent = requester;
        const level = (parent !== undefined) ? (parent.level + 1) : 0;

        const paramsGeometry: TileBuilderParams = {
            extent,
            level,
            segments: layer.segments || 16,
            disableSkirt: layer.disableSkirt,
            hideSkirt: layer.hideSkirt,
            buildIndexAndUv_0: false,
        ...
        return newTileGeometry(builder, paramsGeometry).then((result) => {
            ...
            if (parent) {
                ...
                const geoidHeight =
                    geoidLayerIsVisible(layer) ? tile.geoidHeight : 0;
